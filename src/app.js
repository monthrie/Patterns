...
const STR = {
  pt: {
    ...
    shuffle: "Baralhar", newCombo: "Nova combina\u00e7\u00e3o", saveCols: "Guardar cores",
    palNamePrompt: "Nome desta combina\u00e7\u00e3o:",
  ...
  en: {
    ...
    shuffle: "Shuffle", newCombo: "New combo", saveCols: "Save colours",
    palNamePrompt: "Name this combo:",
...
function KnotMakerMobile() {
  ...
  const [palettes, setPalettes] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('knot.palettes') || '[]');
    } catch {
      return [];
    }
  });
  useEffect(() => {
    try { localStorage.setItem('knot.palettes', JSON.stringify(palettes)); } catch {}
  }, [palettes]);
  ...
  useEffect(() => {
    playerApiRef.current?.setColors?.(cycles.map((_, i) => getCycleColor(i)));
  ...
  const knotViewportEl = /*#__PURE__*/React.createElement("div", {
  ...
  }, playerOpen && /*#__PURE__*/React.createElement("div", {
    ref: playerBoxRef,
    style: {
      position: "absolute", top: 0, left: 0, right: 0, bottom: 0,
      zIndex: 5, background: bgColor
    }
  }), playerOpen && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "absolute", left: "50%", bottom: 12, transform: "translateX(-50%)",
      zIndex: 12, width: "min(720px, calc(100% - 20px))",
      display: "flex", flexDirection: "column", gap: 8,
      padding: "10px 14px",
      background: "rgba(18,22,28,0.95)", border: "1px solid var(--hair)",
    ...
    onClick: () => {
      const a = playerApiRef.current;
      if (!a) return;
      if (a.getState().playing) { a.pause(); setPlayerPaused(true); }
      else { a.play(); setPlayerPaused(false); }
    },
    className: "trans-btn trans-main"
    ...
    className: "trans-btn",
    style: {
      color: playerSpeed === v ? "var(--accent-on-fill)" : "var(--bone-dim)",
      background: playerSpeed === v ? "var(--accent-fill)" : "transparent",
      borderColor: playerSpeed === v ? "var(--accent)" : "var(--hair)",
      minWidth: 40
    }
  ...
  }, T.strand[strandMode]), playerOpen ? null : /*#__PURE__*/React.createElement("button", {
  ...
  const currentCycleColors = () => cycles.map((_, ci2) => getCycleColor(ci2));
  const assignColors = arr => {
    const o = {};
    arr.forEach((c2, ci2) => { o[ci2] = c2; });
    setCycleColors(o);
  };
  const shuffleColors = () => {
    const a2 = currentCycleColors();
    for (let i2 = a2.length - 1; i2 > 0; i2--) {
      const j2 = Math.floor(Math.random() * (i2 + 1));
      const t2 = a2[i2]; a2[i2] = a2[j2]; a2[j2] = t2;
    }
    assignColors(a2);
  };
  const randomCombo = () => {
    const pool = JOAO.filter(c2 => c2.hex !== "#1a1c28").map(c2 => c2.hex);
    for (let i2 = pool.length - 1; i2 > 0; i2--) {
      const j2 = Math.floor(Math.random() * (i2 + 1));
      const t2 = pool[i2]; pool[i2] = pool[j2]; pool[j2] = t2;
    }
    const n2 = 2 + Math.floor(Math.random() * 3);
    assignColors(cycles.map((_, ci2) => pool[ci2 % n2]));
  };
  const applyCombo = p2 => assignColors(cycles.map((_, ci2) => p2.colors[ci2 % p2.colors.length]));
  const saveCombo = () => {
    const name = window.prompt(T.palNamePrompt);
    if (!name || !name.trim()) return;
    const seen = [];
    for (const c2 of currentCycleColors()) if (!seen.includes(c2)) seen.push(c2);
    setPalettes(p2 => [...p2.filter(x2 => x2.name !== name.trim()), { name: name.trim(), colors: seen.slice(0, 8) }]);
  };
  const deleteCombo = name => setPalettes(p2 => p2.filter(x2 => x2.name !== name));
  ...
  }, [[T.shuffle, shuffleColors], [T.newCombo, randomCombo], [T.saveCols, saveCombo]].map(pair => /*#__PURE__*/React.createElement("button", {
    key: pair[0],
    onClick: pair[1],
    style: {
      padding: "9px 14px", background: "transparent",
      border: "1px solid var(--hair)", borderRadius: 8,
      color: "var(--bone-dim)", fontSize: 12, letterSpacing: 0.5,
      cursor: "pointer", fontFamily: "inherit"
    }
  }, pair[0]))), palettes.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: { display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }
  }, palettes.map(p2 => /*#__PURE__*/React.createElement("div", {
    key: p2.name,
    onClick: () => applyCombo(p2),
    style: {
      display: "inline-flex", alignItems: "center", gap: 7,
      padding: "6px 10px", border: "1px solid var(--hair)", borderRadius: 999,
      cursor: "pointer", background: "var(--ink-2)"
    }
  }, /*#__PURE__*/React.createElement("span", {
    style: { display: "inline-flex" }
  }, p2.colors.slice(0, 5).map((c2, k2) => /*#__PURE__*/React.createElement("i", {
    key: k2,
    style: {
      width: 14, height: 14, borderRadius: "50%", background: c2,
      border: "1px solid rgba(0,0,0,0.5)", marginLeft: k2 ? -5 : 0, display: "inline-block"
    }
  }))), /*#__PURE__*/React.createElement("span", {
    style: { fontSize: 12, color: "var(--bone-dim)" }
  }, p2.name), /*#__PURE__*/React.createElement("button", {
    onClick: e => {
      e.stopPropagation();
      if (window.confirm(T.deleteConfirm(p2.name))) deleteCombo(p2.name);
    },
    style: { background: "none", border: "none", color: "var(--oxblood)", cursor: "pointer", fontSize: 13, fontFamily: "inherit", padding: 0 }
  }, "\u2715")))), palettePopover && (() => {
    const pi = palettePopover.i;
    const pvis = isCycleVisible(pi);
    return /*#__PURE__*/React.createElement("div", {
      "data-palette-pop": true,
      style: {
        marginTop: 12, padding: 12,
        background: "var(--ink-2)", border: "1px solid var(--hair)", borderRadius: 10
      }
    }, /*#__PURE__*/React.createElement("div", {
      style: { display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }
    }, /*#__PURE__*/React.createElement("span", {
      style: { width: 18, height: 18, borderRadius: 5, background: getCycleColor(pi), display: "inline-block", border: "1px solid rgba(0,0,0,0.5)", flexShrink: 0 }
    }), /*#__PURE__*/React.createElement("span", {
      style: { fontSize: 12, color: "var(--bone-dim)", textTransform: "uppercase", letterSpacing: 1, whiteSpace: "nowrap" }
    }, T.fio + " " + (pi + 1)), /*#__PURE__*/React.createElement("button", {
      onClick: () => toggleCycle(pi),
      style: {
        marginLeft: "auto", padding: "8px 12px",
        background: pvis ? "transparent" : "var(--accent-fill)",
        border: "1px solid var(--hair)", borderRadius: 8,
        color: pvis ? "var(--bone-dim)" : "var(--accent-on-fill)",
        fontSize: 11, cursor: "pointer", fontFamily: "inherit",
        textTransform: "uppercase", letterSpacing: 0.6, whiteSpace: "nowrap"
      }
    }, pvis ? T.hideFio : T.showFio), /*#__PURE__*/React.createElement("button", {
      onClick: () => setPalettePopover(null),
      style: { padding: "8px 11px", background: "transparent", border: "1px solid var(--hair)", borderRadius: 8, color: "var(--bone)", cursor: "pointer", fontFamily: "inherit" }
    }, "\u2715")), /*#__PURE__*/React.createElement("div", {
      style: { display: "flex", flexWrap: "wrap", gap: 8 }
    }, JOAO.map(sw2 => /*#__PURE__*/React.createElement("button", {
      key: sw2.hex,
      title: sw2.name,
      onClick: () => setCycleColor(pi, sw2.hex),
      style: {
        width: 44, height: 44, padding: 0, background: sw2.hex,
        border: getCycleColor(pi) === sw2.hex ? "2px solid var(--accent-soft)" : "1px solid #00000050",
        cursor: "pointer", borderRadius: 8
      }
    })), /*#__PURE__*/React.createElement("button", {
      onClick: () => colorInputRefs.current[pi]?.click(),
      style: {
        height: 44, padding: "0 14px", background: "transparent",
        border: "1px solid var(--hair)", color: "var(--bone-dim)",
        fontSize: 11, cursor: "pointer", letterSpacing: 1,
        fontFamily: "inherit", textTransform: "uppercase", borderRadius: 8
      }
    }, T.custom)));
  })()) : null;
