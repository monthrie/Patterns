...
// ========== COLOR HELPERS (rope rendering — pure, no engine math) ==========
...
function isLightColor(hex) {
  ...
  return (r * 0.299 + g * 0.587 + b * 0.114) / 255 > 0.55;
}
function ropeGradId(axis, color) {
  return `ropeg-${axis}-${String(color).replace(/[^0-9a-zA-Z]/g, '')}`;
}
...
function KnotMakerMobile() {
  ...
  // Rope rendering: derived under-stroke (outline) colour per strand colour.
  // On light backgrounds darken harder so cream/white yarns stay legible.
  const isLightBg = isLightColor(bgColor);
  const outlineFor = useMemo(() => {
    const cache = new Map();
    const amt = isLightBg ? 0.55 : 0.4;
    const mixT = isLightBg ? 0.08 : 0.15;
    return c => {
      let v = cache.get(c);
      if (!v) {
        v = mixHex(darken(c, amt), bgColor, mixT);
        cache.set(c, v);
      }
      return v;
    };
  ...
  function renderStrandSVG(pts, color, crossingMap) {
    ...
    for (let seg = 0; seg < pts.length - 1; seg++) {
      ...
      // Crossing depth: at every cut, the cut-facing piece-end gets a short
      // dark overlay so the cord reads as diving under the crossing strand.
      const shadeLen = gap * 0.8;
      const pushPiece = (x1, y1, x2, y2, edgeStart, edgeEnd, cutStart, cutEnd) => {
        elements.push({
          type: 'strand',
          x1,
          y1,
        ...
        const sl = Math.min(shadeLen, plen);
        if (sl < 0.5) return;
        if (cutStart) elements.push({
          type: 'shade',
          x1,
          y1,
          x2: x1 + ux * sl,
      ...
      for (const cut of cuts) {
        ...
        if (proj > 0.5) pushPiece(cx, cy, gsx, gsy, isFirst && p1Edge, false, !isFirst, true);
        cx = cut.sx + ux * gap;
        cy = cut.sy + uy * gap;
        isFirst = false;
      }
      const projEnd = (sx2 - cx) * ux + (sy2 - cy) * uy;
      if (projEnd > 0.5) pushPiece(cx, cy, sx2, sy2, isFirst && p1Edge, p2Edge, !isFirst, false);
    }
    return elements;
  }
  ...
  // Rope rendering passes: split pieces by type; collect the active strand
  // colours so one shared repeating gradient per colour+axis can be defined.
  const ropeMode = strandMode !== 'string';
  const strandPieces = useMemo(() => renderElements.filter(el => el.type === 'strand'), [renderElements]);
  const shadePieces = useMemo(() => renderElements.filter(el => el.type === 'shade'), [renderElements]);
  const ropeColors = useMemo(() => {
    if (!ropeMode) return [];
    const set = new Set();
    for (const el of strandPieces) if (el.axis) set.add(el.color);
    return Array.from(set);
  }, [strandPieces, ropeMode]);
  const paintFor = el => ropeMode && el.axis ? `url(#${ropeGradId(el.axis, el.color)})` : el.color;
  ...
  const knotViewportEl = /*#__PURE__*/React.createElement("div", {
    ...
    // Rope-cylinder shading: ONE shared repeating gradient per active colour
    // per diagonal axis. Parallel strand centre-lines are spaced cellSz/√2
    // apart, so the gradient vector runs along the axis normal with that
    // period, anchored at grid point (0.5, 0) — it lies on a centre-line of
    // both the "\" (y−x=−0.5) and "/" (y+x=0.5) families, so offset 0 always
    // falls ON a strand centre-line.
    const ax = toX(0.5),
      ay = toY(0);
    const lt = lighten(c, 0.22),
      dk = darken(c, 0.28);
    return ['bs', 'fs'].map(axis => /*#__PURE__*/React.createElement("linearGradient", {
      key: ropeGradId(axis, c),
      id: ropeGradId(axis, c),
      gradientUnits: "userSpaceOnUse",
      spreadMethod: "repeat",
      x1: ax,
      y1: ay,
      x2: ax + cellSz / 2,
      y2: axis === 'bs' ? ay - cellSz / 2 : ay + cellSz / 2
    }, /*#__PURE__*/React.createElement("stop", {
      offset: "0%",
      stopColor: lt
    }), /*#__PURE__*/React.createElement("stop", {
    ...
    stroke: "#3a2f1f",
    strokeWidth: 1.5
  })), ropeMode && strandPieces.map((el, i) => /*#__PURE__*/React.createElement("line", {
    key: `u${i}`,
    x1: el.x1,
    y1: el.y1,
    x2: el.x2,
    y2: el.y2,
    stroke: outlineFor(el.color),
    strokeWidth: el.w + 2.5,
    strokeLinecap: "butt"
  })), ropeMode && cornerArcs.map((arc, i) => /*#__PURE__*/React.createElement("path", {
    key: `cau${i}`,
    d: `M${arc.sx1},${arc.sy1} A${arc.r},${arc.r} 0 0,${arc.sweep} ${arc.sx2},${arc.sy2}`,
    fill: "none",
    stroke: outlineFor(arc.color),
    strokeWidth: sw + 2.5,
    strokeLinecap: "butt"
  })), ropeMode && strandPieces.filter(el => el.edgeStart || el.edgeEnd).map((el, i) => {
    const r = el.w / 2 + 1.25;
    const caps = [];
    ...
    return caps;
  }), strandPieces.map((el, i) => /*#__PURE__*/React.createElement("line", {
    key: i,
    x1: el.x1,
    y1: el.y1,
    x2: el.x2,
    ...
    strokeWidth: sw,
    strokeLinecap: "butt"
  })), strandPieces.filter(el => el.edgeStart || el.edgeEnd).map((el, i) => {
    const r = el.w / 2;
    const caps = [];
    if (el.edgeStart) caps.push(/*#__PURE__*/React.createElement("circle", {
      key: `es${i}`,
    ...
    return caps;
  }), shadePieces.map((el, i) => /*#__PURE__*/React.createElement("line", {
    key: `sh${i}`,
    x1: el.x1,
    y1: el.y1,
    x2: el.x2,
    ...
    // Brass pinheads: shared radial gradient, soft drop shadow, white glint.
    // Corner pins are subtly larger than edge pins.
    const nx = toX(n.x),
      ny = toY(n.y);
    const r = n.type === 'corner' ? 4.2 : 3.5;
    return /*#__PURE__*/React.createElement("g", {
      key: `nail${i}`,
      style: {
        pointerEvents: "none"
